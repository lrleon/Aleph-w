...
- **MUST**: Use these Aleph exception macros (defined in `ah-errors.H`):
  ...
  - `ah_out_of_range_error_if(condition)` → for out-of-range errors (replaces `throw std::out_of_range_error`)
...
- **Example (Aleph style - CORRECT)**:
  ...
  ah_out_of_range_error_if << "Index " << index << " out of range [0, " << size << ")";
