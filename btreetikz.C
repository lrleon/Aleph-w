...
void load_key_option(ifstream& input_stream)
{
  ...
  if (position >= prefix_dynarray.size())
    PRINT_AH_ERROR("Node position greater than number of nodes in KEY option");
...
void load_rectangle_option(ifstream& input_stream)
{
  ...
  if (height == 0)
    PRINT_AH_ERROR("Heigth in nodes cannot be zero");
...
void load_distance_option(ifstream& input_stream)
{
  ...
  if (token_type != LEFT and token_type != RIGHT)
    PRINT_AH_ERROR("Invalid orientation in DISTANCE option");
...
void assign_triangle(EepicNode<long> * p, int, int position)
{
  ...
  if (found_index >= 0)
	...
	PRINT_AH_ERROR("Triagle on %d th node is not a leaf", INFIXPOS(p));
...
void assign_rectangle(EepicNode<long> * p, int, int position)
{
  ...
  if (found_index >= 0)
    {
      if (not (LLINK(p) == NULL and RLINK(p) == NULL))
	PRINT_AH_ERROR("Rectangle on %d th node is not a leaf", INFIXPOS(p));
...
void assign_distance(EepicNode<long> * p, int, int position)
{
  ...
  if (found_index >= 0)
	...
	PRINT_AH_ERROR("Distance line on %d th node has a left branch",
		    INFIXPOS(p));
      if (line_distance_data.orientation == RIGHT and RLINK(p) != NULL)
	PRINT_AH_ERROR("Distance line on %d th node has a right branch",
		    INFIXPOS(p));
...
void file_to_dynarrays(const char * file_name)
{
  try
      ...
      while (true)
	{
	  ...
	  switch (token_type)
	    {
	    case INVALID: PRINT_AH_ERROR("Found an invalid token");
	    case END_FILE: 
	      return;
		...
		case AUX:
		  ...
		  break;
		default: PRINT_AH_ERROR("Found an string in invalid mode (%ld)", 
				     parsing_state);
		} 
	      break;
	    ...
	    default:
	      PRINT_AH_ERROR("Internal error: invalid token type (%ld)", 
			  token_type);
	    }
	}
    }
			       ...
			       const size_t &    level)
{
  ...
  compute_nodes_in_level(root, level, deepest_nodes);
...
void generate_prologue(ofstream & output)
{
  ...
  output << endl
	 ...
	 << "%      generated by btreetikz program" << endl
	 << endl
	 << "% Copyright (C) 2011" << endl
	 << "% UNIVERSITY of LOS ANDES (ULA)" << endl
	 << "% Merida - REPÚBLICA BOLIVARIANA DE VENEZUELA" << endl
	 ...
	 << endl
	 << "% You must use tikz latex package" << endl
	 << "% in your LaTeX application" << endl
	 << endl
	 << "% PGF/TikZ was develeopped Till Tantau and Christian Feuersaenger" 
	 << endl
  ...
  if (latex_header)
   ...
   output << endl
	  ...
	  << "\\begin{tikzpicture}(" << h_size << "," << v_size << ")" 
...
void generate_tree(ofstream & output, EepicNode<long> * p)
{
  ...
  if (ISEXTERNAL(p)) /* recta correspondiente a nodo externo */
    output << endl 
           << "%   External node" << endl
	   << "\\draw (" << x - hr << "pt," << YPIC(y) << "pt) -- (" 
	   << x + hr << "pt," << YPIC(y) << "pt) ;" << endl;
  else if (ISTRIANGLE(p)) /* dibujar triangulo */
    output << endl
	   << "%   Triangle" << endl
	   << "\\draw (" << x << "pt," << YPIC(y) << "pt) -- ("
	   << x - hd << "pt," << YPIC(y + TRIANGLEH(p)) << "pt) -- ("
	   << x + hd << "pt," << YPIC(y + TRIANGLEH(p)) << "pt) -- ("
	   << x << "pt," << YPIC(y) << "pt) -- cycle ;";
      ...
      if (SCRATCH(p))
	{
	  ...
	  output << endl
		 ...
		 << "\\draw (" << x1 << "pt," << YPIC(y1) << "pt) -- ("
		 << x2 << "pt," << YPIC(y1) << "pt) -- ("
		 << x2 << "pt," << YPIC(y2) << "pt) -- ("
		 << x1 << "pt," << YPIC(y2) << "pt) -- ("
		 << x1 << "pt," << YPIC(y1) << "pt) -- cycle ;";
	  ...
	  output << endl
		 ...
		 << x1 << "pt," << YPIC(y2) << "pt) -- ("
		 << x2 << "pt," << YPIC(y2) << "pt) -- ("
		 << x2 << "pt," << YPIC(y1) << "pt) -- cycle ;" << endl
		 << "\\draw (" << x1 << "pt," << YPIC(y1) << "pt) -- ("
		 << x2 << "pt," << YPIC(y2) << "pt)" << endl
		 << "\\draw(" << x2 << "pt," << YPIC(y1) << "pt) -- ("
		 << x1 << "pt," << YPIC(y2) << "pt) ; ";	    
	}
      else
	{
    ...
    output << endl
	   << "%   Elippse" << endl
	   << "\\draw" << (SHADOW(p) ? "[shadow" : "")
	   <<" (" << x << "pt," << YPIC(y) << "pt) " 
	   << "ellipse (" << hd << "pt and " << vd << "pt) ;";
  else if (SHADOW(p))
    output << endl
	   << "%   Elippse" << endl
	   << "\\draw[shadow] (" << x << "pt," << YPIC(y) << "pt){\\ellipse*{" << hd 
	   << "}{" << vd << "}}";
  ...
  if (ISDISTANCE(p))
	...
	PRINT_AH_ERROR("Distance line on %d th complete node", INFIXPOS(p));
      ...
      output << endl
	     << "\\draw (" << xl << "pt," << YPIC(y - yplus) << "pt) -- (" 
	     << xl << "pt," << YPIC(y + line_len) << "pt) ; " << endl
	     << "\\draw (" << xl << "pt," << YPIC(yf1 + yof) << "pt) -- ("
	     << xl << "pt," << YPIC(yf2 + yplus) << "pt) ; " << endl
	     << "\\draw (" << xl - gap_len/2 << "pt," << YPIC(y) << "pt) -- ("
	     << xl + gap_len/2 << "pt," << YPIC(y) << "pt) ; " << endl
	     << "\\draw (" << xl - gap_len/2 << "pt," << YPIC(yf2 + yplus) 
	     << "pt) ; " << endl
	     << "\\draw (" << xl - gap_len/2 << "pt," << YPIC(yf2) << "pt) -- ("
	     << xl + gap_len/2 << "pt," << YPIC(yf2) << "pt); " << endl
	     << "\\draw (" << xl - gap_len/2 << "pt," << YPIC(y + dy) << "pt) -- ("
	     << xl + gap_len/2 << "pt," << YPIC(y - dy) << "pt) ;" << endl
	     << "\\draw (" << xl - gap_len/2 << "pt," << YPIC(yf2 + dy) << "pt) -- ("
	     << xl + gap_len/2 << "pt," << YPIC(yf2 - dy) << "pt) ;";
	   ...
	   itor.has_current(); itor.next())
	{
	  ...
	  switch (tag_data.tag_option)
		...
		string_width(tag_data.tag); 
	      ...
	      break;
	    default:
	      PRINT_AH_ERROR("Internal error: unexpected tag option");
	    }
  ...
  if (SCRATCH(p) and not ISRECTANGLE(p))
    {
      ...
      output << endl
	     << "%   Scratching" << endl
	     << "\\draw (" << x - dr << "pt," << YPIC(y - dr) << "pt)"
	     << "(" << x + dr << "pt," << YPIC(y + dr) << "pt)"
	     << "\\draw (" << x + dr << "pt," << YPIC(y - dr) << "pt)"
	     << "(" << x - dr << "pt," << YPIC(y + dr) << "pt)";
    }
  ...
  for (DynDlist<Arc_Desc>::Iterator itor(ARCLIST(p)); itor.has_current(); 
	...
	{ // si hay triangulo o rectangulo arc va hasta la punta de triangulo
      ...
      draw_arc_tikz(output, src_x, src_y, tgt_x, tgt_y, 
  ...
  if (l != NULL and WITHARC(l))
	...
	draw_arc_tikz(output, x - dx, y + dy, lx, ly, DASHLLINK(p), with_arrow);
      else
	draw_arc_tikz(output, x - dx, y + dy, lx + dx, ly - dy, 
	...
	output << "\\curve(" << x - dx << "pt," << YPIC(y + dy) << "pt," 
	       << px + dx << "pt," << YPIC(y + h) << "pt,"
	       << px + dx << "pt," << YPIC(py + dy) << "pt)";
      else
	output << "\\spline(" << x - dx << "pt," << YPIC(y + dy) << "pt)(" 
	       << px << "pt," << YPIC(y + h) << "pt)("
	       << px + dx << "pt," << YPIC(py + dy) << "pt)";
    }
  ...
  if (r != NULL and WITHARC(r))
	...
	draw_arc_tikz(output, x + dx, y + dy, rx, ry, DASHRLINK(p), with_arrow);
      else
	draw_arc_tikz(output, x + dx, y + dy, rx - dx, ry - dy, 
...
void generate_split_lines(ofstream & output, EepicNode<long> * root)
{
  if (split_dynarray.size() > num_nodes)
    PRINT_AH_ERROR("Number of split points is greater than total of nodes");

  for (int i = 0; i < split_dynarray.size(); i++)
    {
      int pos = split_dynarray[i];
      if (pos >= num_nodes - 1)
	PRINT_AH_ERROR("Split position (%d) out of range", pos);

      EepicNode<long> * src = select(root, pos);
      EepicNode<long> * tgt = select(root, pos + 1);
      ...
      output << endl
	     ...
	     << "\\dashline{" << dash_len << "}(" << x << "pt," 
	     << YPIC(line_space) << "pt)"
	     << "(" << x << "pt," << YPIC(line_space + line_len) << "pt)";
      ...
      if (upper_len > 0) 
	{
	  ...
	  output << endl
		 << "\\put(" << x - dx << "pt," << YPIC(line_space) << "pt)"
      ...
      if (lower_len > 0)
	{
	  ...
	  output << endl
		 << "\\put(" << x - dx << "pt," 
		 << YPIC(line_space + line_len + font_height) << "pt)"
...
static struct argp_option options [] = {
  ...
  {"aux-x-offset", 'H', "offset", OPTION_ARG_OPTIONAL, 
...
static error_t parser_opt(int key, char *arg, struct argp_state *)
{
  switch (key)
	...
	if (pos == string::npos)
	  output_file_name = input_file_name + ".tikz";
	else
	  output_file_name = 
	    string(input_file_name).replace(pos, 
					    input_file_name.size() - pos, 
					    string(".tikz"));
	break;
      }
