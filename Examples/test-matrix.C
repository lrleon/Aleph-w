...
int main(int argc, char *argv[])
{
  ...
  const AlDomain<string> rd = {"A", "B", "C"};
  const AlDomain<string> cd = {"A", "B", "C"};
  ...
  Matrix<string, string> m2(rd, cd, {{1, 2, 3}, {4, 5, 6}, {7, 8, 9}});
  ...
  m2.to_rowlist().for_each([] (const Vector<string, double> & v)
                           {
                             cout << v.to_str() << endl << endl;
                           });
  cout << endl
       << "The column vectors = " << endl;
  m2.to_collist().for_each([] (const Vector<string, double> & v)
                           {
                             cout << v.to_str() << endl << endl;
                           });
  cout << endl;
  ...
  m3.row_domain_list().
    for_each([&m3] (const std::string & row)
             {
  ...
  m3.col_domain_list().
    for_each([&m3] (const std::string & col)
             {
  ...
  assert(zip(m3.row_domain_list(), m3.col_domain_list()).all
         ([&m3, &m3t] (const std::pair<std::string, std::string> & p)
          {
  ...
  m3.get_col_domain().keys().
    for_each([&m3] (const string & col)
             {
  ...
  {
    const AlDomain<string> R = {"A", "B", "C", "D"};
    const AlDomain<string> C = {"a", "b", "c"};
