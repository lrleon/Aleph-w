...
void example_basic_usage()
{
  ...
  // Create a compact quotient filter with 2^10 slots (1024) and 8-bit remainders
...
void example_memory_optimization()
{
  ...
  cout << "  - Memory: " << compact.memory_usage() << " bytes ("
       << (compact.memory_usage() / 1024.0 / 1024.0) << " MB)\n";
...
void example_deletion()
{
  ...
  for (const auto& sid : sessions)
    cout << "  " << sid << ": " 
         << (active_sessions.contains(sid) ? "ACTIVE" : "EXPIRED") << "\n";
}
