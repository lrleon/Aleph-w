...
ValueArg<size_t> n =
    {
      "n", "n", "number of items", false, 1000,
      "number of items", cmd
    };

ValueArg<double> p =
    {
      "p", "prob", "probability of failures", false, .01,
      "probability of failures", cmd
    };

ValueArg<unsigned long> seed =
  {"s", "seed", "seed", false, 0, "seed", cmd};
...
int main(int argc, char *argv[])
{
  ...
  const auto t = f.estimate(n.getValue(), p.getValue());
  const auto m = get<0>(t);
  ...
  for (auto i = 0; i < 10 * n.getValue(); ++i)
    if (long val = gsl_rng_get(r); val >= 0 and val < n.getValue())
      {
